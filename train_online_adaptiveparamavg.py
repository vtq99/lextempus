...
for idx in (tqdm(range(len(samples)), desc=f"Training")):
    ...
    if (idx > 1000) and (idx % 100 == 0):
        old_samples = np.array(test_ppl_all[-1100: -100])
        old_mean = np.nanmean(old_samples)
        old_std = np.nanstd(old_samples)
        if old_std != 0:
            new_samples = np.array(test_ppl_all[-100:])
            z = (new_samples - old_mean) / old_std
            z_score = np.nanmin(z)
            if (z_score < -1.4) and (idx > (int(adapter_names[-1]) + 1000)):
            # if (z_score < -1.15) and (idx > (int(adapter_names[-1]) + 1000)):
                print('New adapter add at: ', idx)
                adapter_names.append(str(idx))
    ...
    if str(idx) in adapter_names[1:]:
        ...
        pickle.dump(ppl, open(f'./checkpoints/echr_online_paramavg_zscore_2.pkl', 'wb'))
        # pickle.dump(ppl, open(f'./checkpoints/eu_online_paramavg_zscore_115.pkl', 'wb'))
        model.save_adapter(f'./checkpoints/paramavg1_{adapter_names[-2]}.pt', adapter_names[-2])
        for name, child in model.named_children():
            if 'lm_head' not in name:
                for param in child.parameters():
                    param.requires_grad = False
        model.load_adapter(f'./checkpoints/paramavg1_{adapter_names[-2]}.pt', load_as=adapter_names[-1], with_head=False)
        model.active_adapters = adapter_names[-1]
        optimizer = torch.optim.Adam(model.parameters(),
                                     lr=deepspeed_config["optimizer"]["params"]["lr"],
                                     weight_decay=deepspeed_config["optimizer"]["params"]["weight_decay"]
                                     )
    ...
    #     # distance.append(d.min())
    #     w = softmax(0 - np.array(distance))
    #     w = w.tolist() + [1/(len(adapter_names) -2)]
    w = [2**x for x in range(len(adapter_names))]
...
pickle.dump(ppl, open(f'./checkpoints/echr_online_paramavg_zscore_2.pkl', 'wb'))
# pickle.dump(ppl, open(f'./checkpoints/eu_online_paramavg_zscore_115.pkl', 'wb'))
model.save_pretrained(f'./checkpoints/echr_online_paramavg_zscore_2.pt')
