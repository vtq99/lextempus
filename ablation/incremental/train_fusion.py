...
adapter_names = [str(i) for i in train_years]
...
#         # torch.save(model.state_dict(), f'./checkpoints/thesis_deepspeed2_{year}')
#         # Save model
#         model.save_pretrained(f'./checkpoints/thesis_deepspeed2_{year}.pt')
#         # Save adapter
#         model.save_adapter(f'./checkpoints/adapter_{year}.pt', "bottleneck_adapter")
#
# print(ppl_dct)
#
# loss_all_year = []
...
for year in train_years:
    model.load_adapter(f'./checkpoints/adapter_{year}.pt', load_as=f'{year}', with_head=False)
print(f'#Param: {sum(p.numel() for p in model.parameters() if p.requires_grad)}')
adapter_setup = ac.Fuse(*adapter_names)
model.add_adapter_fusion(adapter_setup)
model.set_active_adapters(adapter_setup)
model.train_adapter_fusion(adapter_setup)
# model.add_adapter_fusion(adapter_names[:i + 1])
# model.active_adapters = ac.Fuse(*adapter_names[:i + 1])
print(f'#Param: {sum(p.numel() for p in model.parameters() if p.requires_grad)}')
model.to(device)
# for name, child in model.named_children():
#     if 'lm_head' not in name:
...
for i, year in enumerate(train_years):
    ...
    for epoch in range(1, 5):
        ...
        model.save_pretrained(f'./checkpoints/thesis_deepspeed2_fusion.pt')
    ...
    perplexity = math.exp(average_loss)
    print(f'All year loss: {average_loss}, All year ppl: {perplexity}')
model.save_adapter(f'./checkpoints/adapter_fusion.pt', "adapter")