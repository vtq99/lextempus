...
for i, year in enumerate(train_years):
    ...
    if i > 0:
        samples_replay = []
        for y in train_years[:i]:
            samples_replay += samples[y]
        replay_dataset = MyDataset(tokenizer, samples_replay)
    ...
    for epoch in range(1, 4):
        ...
        torch.save(model.state_dict(), f'./checkpoints/thesis_er_{year}')
