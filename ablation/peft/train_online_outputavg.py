...
model.add_adapter('0')
model.active_adapters = ac.Average('0', weights=[1])
...
for idx in (tqdm(range(len(samples)), desc=f"Training")):
    ...
    if str(idx) in adapter_names[1:]:
        ...
        pickle.dump(ppl, open(f'./checkpoints/echr_online_outputavg4000.pkl', 'wb'))
        new_adapters = adapter_names[:adapter_names.index(str(idx)) + 1]
        model.save_adapter(f'./checkpoints/outputavg_{new_adapters[-2]}.pt', new_adapters[-2])
        # for name, child in model.named_children():
        #     if 'lm_head' not in name:
        #         for param in child.parameters():
        #             param.requires_grad = False
        model.load_adapter(f'./checkpoints/outputavg_{new_adapters[-2]}.pt', load_as=new_adapters[-1], with_head=False)
        model.active_adapters = ac.Average(*new_adapters, weights=[2 ** i for i in range(len(new_adapters))])
...
ppl = {'train': ppl_all, 'test': test_ppl_all}
pickle.dump(ppl, open(f'./checkpoints/echr_online_outputavg4000.pkl', 'wb'))
model.save_pretrained(f'./checkpoints/echr_online_outputavg4000.pt')

for adapter_name in adapter_names:
    model.save_adapter(f'./checkpoints/outputavg_{adapter_name}.pt', adapter_name)
