...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        tokens = self.tokenizer(text,
                                padding='max_length', truncation=True, max_length=994,
...
class TestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        tokens = self.tokenizer(f' {tokenizer.eos_token} '.join(text[:-1]) + f' {tokenizer.eos_token}',
                                padding='max_length', truncation=True, max_length=482,
                                return_tensors='pt')
...
model.save_pretrained(f'./checkpoints/echr_online_prefixfreeze5_rec.pt')
import pickle
ppl = {'train': ppl_all, 'test': test_ppl_all}
pickle.dump(ppl, open(f'./checkpoints/echr_online_prefixfreeze5_rec.pkl', 'wb'))
