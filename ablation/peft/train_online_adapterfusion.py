...
model.add_adapter('0')
model.add_adapter_fusion(['0'])
model.active_adapters = ac.Fuse('0')
...
for idx in (tqdm(range(len(samples)), desc=f"Training")):
    ...
    if str(idx) in adapter_names[1:]:
        ...
        model.add_adapter_fusion(new_adapters)
        model.active_adapters = ac.Fuse(*new_adapters)
...
model.save_pretrained(f'./checkpoints/echr_online_fuse4000.pt')
import pickle

ppl = {'train': ppl_all, 'test': test_ppl_all}
pickle.dump(ppl, open(f'./checkpoints/echr_online_fuse4000.pkl', 'wb'))

for adapter_name in adapter_names:
    model.save_adapter(f'./checkpoints/adapterfusion_{adapter_name}.pt', adapter_name)
