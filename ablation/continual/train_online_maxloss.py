...
for idx in (tqdm(range(len(samples)), desc=f"Training")):
    ...
    if idx % 10 == 0 and idx > 0:
        indices = np.random.choice(np.arange(max(0, idx-512), idx), size=10, replace=False)
        r = []
        for item in indices:
            model.eval()
            for batch in test_dataloaders[idx]:
                ...
                with torch.no_grad():
                    ...
                    test_loss_all.append(loss.item())
            average_loss = np.nanmean(test_loss_all)
            r.append(average_loss)
        orders = np.array(r).argsort()
        ranks = orders.argsort()
        indices = indices[ranks[:3][::-1]]
        for item in indices:
...
model.save_pretrained(f'./checkpoints/echr_online_maxloss.pt')
import pickle

ppl = {'train': ppl_all, 'test': test_ppl_all}
pickle.dump(ppl, open(f'./checkpoints/echr_online_maxloss.pkl', 'wb'))
